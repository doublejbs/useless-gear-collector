...
export function stripHtmlNoise(html: string, maxBytes = MAX_HTML_BYTES): string {
  const $ = cheerio.load(html);
  $("script, style, nav, footer, header, iframe, noscript").remove();
  const cleaned = $.html();
  return Buffer.from(cleaned, "utf8").slice(0, maxBytes).toString("utf8");
}
...
export class AIAgentAdapter {
  ...
  async fetchProductsFromSite(
    ...
    while (currentUrl && !visitedUrls.has(currentUrl) && pageCount < maxPages) {
      ...
      try {
        const cleaned = stripHtmlNoise(html, 30_000);
        const message = await this.client.messages.create({
          model: MODEL,
          max_tokens: 2048,
